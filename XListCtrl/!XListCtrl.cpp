...
void XCListCtrl::OnDropFiles(HDROP hDropInfo) 
{
	...
	{
		ZeroMemory(DropFileName,_MAX_PATH+1);
		DragQueryFile(hDropInfo, s, (LPTSTR)DropFileName,_MAX_PATH+1);

		CSDropFileName=DropFileName;
//		XInsertData(s+indx,CSDropFileName,0,-1);
		if ( (sr=CSDropFileName.ReverseFind('\\'))==-1)
		{
			sr=CSDropFileName.ReverseFind('/');
		}
		CSDropFileName=CSDropFileName.Right(CSDropFileName.GetLength()-sr-1);
//		InsertItem(s+indx,CSDropFileName,NULL);
//		SetItemText(s+indx,1,"        ");
//		SetItemText(s+indx,2,"        ");
		AfxMessageBox(CSDropFileName);
	}
...
/*
	...
	int indx=HitTest(pit,&HitTest_flags);
	...
	ScrollPos=FlatSB_GetScrollPos(this->m_hWnd,SB_VERT);
	// update scrollbar
	ScrollDown->XUpdate();
	ScrollUp->XUpdate();
	ScrollLine->XUpdate();
